...
{	if (this.fade_) 
	...
	{	var sc=0;
		...
		try{
			...
			//if (style[i].getZIndex()<0) e.vectorContext.drawGeometry(shadow||geom); //heyuan 20190319
			if (style[i].getZIndex()<0) e.vectorContext.drawGeometry(geom); 
	...
	{	case "donut":
		...
		//	var s = Math.min(5,2*this.radius_/this.data_.length);
			...
			{	context.beginPath();
				context.fillStyle = this.colors_[i%this.colors_.length];
			//	x = x0 + s;
				var h = this.data_[i]/max*canvas.height;
